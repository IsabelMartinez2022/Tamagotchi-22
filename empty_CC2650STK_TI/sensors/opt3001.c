...
double opt3001_get_data(I2C_Handle *i2c) {
    ...
    //Complete this function to return the measured value as lux

	double lux = -1.0; // return value of the function

	// JTKJ: Find out the correct buffer sizes (n) with this sensor?
	uint8_t txBuffer[1]; // Transmission buffer
	uint8_t rxBuffer[2]; // Reception buffer
    // uint8_t txBuffer[ n ];
    // uint8_t rxBuffer{ n ];

	// Fill in the i2cMessage data structure with correct values
	...
	if (opt3001_get_status(i2c) & OPT3001_DATA_READY) {
		...
		if (I2C_transfer(*i2c, &i2cMessage)) {
		    ...
		    //Here the conversion from register value to lux
		    uint16_t rxBufferLonger;
		    uint16_t rxBufferReg;

		    rxBufferLonger= rxBuffer[0] <<8;
		    rxBufferReg= (rxBufferLonger | rxBuffer[1]);

            uint16_t sol1= rxBufferReg &(~(0b0000111111111111));
            sol1= sol1>>12;

            uint16_t sol2 = rxBufferReg &(~(0b1111000000000000));
		    lux = 0.01 * pow(2,sol1) * sol2;
